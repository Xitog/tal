...
Table des matières [MAT]
  ...
  3. Bilan de l'annotation [BIL]
        ...
        3.6 Résultats
            ...
            3.6.3 Accord inter-annotateur pour la feature autoref
            3.6.4 Accord inter-annotateur total
    [FIN]
...
2.6 Conversion XML->AA/AC
-------------------------

Le fichier script1.py contient le code de traduction de l'XML TEI-P5 vers un fichier corpus Glozz (AC) et un fichier annotation (AA). Ce script est un travail original.
...
Cette systématicité permet de mieux comprendre l'enchaînement des discussions. Nous utilisons les unités typographiques par défaut de Glozz pour produire un document plus lisible. Un mode debug permet de rajouter des lignes dans le document produit pour Glozz pour encore mieux visualiser les différentes parties de la page de discussion.
...
--------------------------
    ...
    o Nous avons rajouté dans notre annotation deux unités << Signature >> que le script de transformation XML -> AA/AC n'avait pas trouvées, Silvia ne l'a pas fait, nous ne les comparons donc pas.

3.3 Correspondance de positions
-------------------------------

Pour savoir quoi comparer ensemble, il faut constituer des paires d'unités et pour cela nous utilisons la position des unités. Nous considérons qu'il existe une marge d'erreur entre les positions de deux unités. La sélection des limites se faisant à la souris, de manière peu précise, l'utilisateur peut toujours déborder d'un caractère ou deux. Nous considérons ainsi deux unités en correspondance de positions si les différences absolues entre leurs bornes start et leurs bornes end sont inférieures à une précision donnée, que nous avons mis empiriquement à deux. Nous ne testons pas le contenu des chaînes délimitées par les bornes car ces positions se réfèrent à un même corpus. Prenons un exemple :
...
On peut ensuite passer comparer les valeurs des features de celles en correspondance de positions.
...
On peut ensuite, pour les unités en correspondance de positions, évaluer l'inter-annotateur sur les différentes features définies pour les unités.
...
Les unités typographiques étant générées automatiquement par le script de conversion (script1.py), il est normal qu'elles soient identiques et qu'on en compte donc le même nombre. J'ai rajouté manuellement deux << Signature >> comment indiqué plus haut. J'ai étiqueté 73 unités contre 59 pour Silvia.
...
En regardant les textes des différents unités sans correspondance, il s'agit toujours de pronoms ou d'identifiants d'utilisateurs, donc des unités qui sont valides, mais qui n'ont pas été repérées par un des deux annotateurs.
