...
# Usage :
# Load the downloaded notice from HAL in memory and perform counting:
#   Length of title in words and char, first word of title, first word after :
#   Special char
# Options :
#   variable GENERATE_LEXICON (True or False) : generate the lexicon of words 7
#                                               used in title with frequencies.
#   variable SEARCH_ENGLISH_TITLES (True or False) : check the language used
#                                  (slow)            in suspect title.
#   variable REPO_DUMP (True or False) : serialize the in memory Repository
#                                        in XML or JSON and can produce a zip.
#   variable NLP (None or StanfordCoreNLP instance) : perform tokenization and
#                         (very slow)                 pos tagging using SCoreNLP
#===============================================================================
...
class Repository:
    ...
    def discarded_info(self):
        ...
        print(f'    {len(self.id_discard)} where also discarded as specified by the parameter of the constructor.')
...
class Title:
    ...
    def from_raw_json(dic, filename, nlp=None, count_special_char=False):
        ...
        t.domains = Domain.register_list(t.raw_domains, t)
        # Title
...
if __name__ == '__main__':
    ...
    excel = ExcelFile(name='results2', mode='w')
